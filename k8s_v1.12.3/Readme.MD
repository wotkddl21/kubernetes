...
|output |stdout으로 출력<br>not 'Healthy' componenestatuses, <br>not 'Ready' nodes,<br>not 'Bound' pv,<br>not 'Normal' events,<br>not 'Bound' pvc,<br>[ clusterrolebindings, clusterroles, namespaces, priorityclasses, controllerrevisions, endpoints,  horizontalpodautoscalers, ingresses, rolebindings, roles, secrets, serviceaccounts, configmaps] <br> - > ./$1/backup_latest/get/*에 저장된 값중 지금 존재하지 않는 resource를 출력<br>DESIRED!= CURRENT or DESRIED!= READY or DESIRED!= AVAILABLE인 daemonsets,<br>DESIRED != CURRENT or DESIRED != AVAILABLE 인 deployments,<br>DESIRED != CURRENT or DESIRED != READY 인 replicasets,<br>DESIRED != CURRENT 인 statefulsets |
